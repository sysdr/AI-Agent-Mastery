...
async def lifespan(app: FastAPI):
    # Startup
    global gateway, auth_manager, threat_detector, metrics, logger
...
async def login(request: Request):
    ...
    try:
        ...
        print(f"Login attempt: {body}")  # Use print for debugging
        if not auth_manager:
            raise HTTPException(status_code=503, detail="Auth manager not available")
        
        result = await auth_manager.authenticate(body.get("username"), body.get("password"))
        if result:
            return result
        raise HTTPException(status_code=401, detail="Authentication failed")
    except Exception as e:
        print(f"Login error: {str(e)}")  # Use print for debugging
